"""32-band spectrum analyzer screen."""
    ...
    def draw(self, surface: pygame.Surface):
        ...
        for i in range(NUM_BANDS):
            ...
            if bar_h > 0:
                ...
                segments = max(1, bar_h // 4)
                for s in range(segments):
                    frac = s / max(1, segments - 1)
                    sy = BAR_AREA_Y + BAR_AREA_H - (s + 1) * (bar_h // segments)
                    sh = max(1, bar_h // segments)
                    if frac > 0.85:
                        color = RED
                    elif frac > 0.6:
                        color = YELLOW
                        ...
                        color = GREEN
                    pygame.draw.rect(surface, color, (x, sy, BAR_W, sh))
