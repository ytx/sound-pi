...
- name: Setup Sound-Pi
  ...
  roles:
    ...
    - spi-lcd             # 新規: SPI LCD (ILI9486) DRM + XPT2046 タッチ
...
`/dev/dri/card*` に独立した DRM デバイスを作成する。
...
3. HDMI 出力の無効化（`hdmi_ignore_hotplug`）
   - vc4 が HDMI フレームバッファを作成しようとする `failed to add fb -22` スパムを防止
   - これにより Xorg の起動が数秒に短縮（無効化しないと 3〜5 分かかる）
4. Xorg が SPI LCD をプライマリディスプレイとして使用する設定
   - `/etc/X11/xorg.conf.d/10-spi-lcd.conf` の配置
   - `kmsdev` で `/dev/dri/by-path/` シンボリックリンクを使用（card 番号は不安定）
5. タッチキャリブレーション設定の配置（udev ルール）

**表示パイプライン（Trixie DRM 方式）:**
```
X11 (Electron) → modesetting drv → /dev/dri/by-path/platform-fe204000.spi-cs-0-card → SPI → LCD
```

**DRM デバイス構成（Pi 4 + piscreen overlay）:**
```
/dev/dri/card* — V3D (3Dアクセラレータ)       ※card番号は起動毎に変わりうる
/dev/dri/card* — ili9486 (SPI LCD)            ← Xorg は kmsdev で by-path 指定
/dev/dri/card* — vc4-drm (HDMI)              ← rp1-test.service が PrimaryGPU 設定
```

**安定した DRM デバイスパス:**
```
/dev/dri/by-path/platform-fe204000.spi-cs-0-card  → SPI LCD (ili9486)
```
...
Section "Device"
  Identifier "SPI-LCD"
  Driver "modesetting"
  Option "kmsdev" "/dev/dri/by-path/platform-fe204000.spi-cs-0-card"
...
**`99-v3d.conf` は rp1-test.service が管理:**
- `rp1-test.service` が毎回起動時に `99-v3d.conf` を `PrimaryGPU "true"` で生成
- これは **必要な動作**: logind が vc4 で DRM master を取り、SPI LCD の fd が空く
- rp1-test.service をマスクすると drmSetMaster が失敗するため、マスクしないこと
...
- card 番号はデバイス検出順に依存し不安定 → `/dev/dri/by-path/` を使用
...
- `rp1-test.service` は vc4 に PrimaryGPU を設定する重要なサービス → マスクしない
...
6. `.xinitrc` 配置（Electron kiosk 起動）
   - `--kiosk --no-sandbox --window-size=480,320 --disable-gpu --disable-gpu-compositing --ozone-platform=x11`
   - `--disable-software-rasterizer` は使用禁止（白画面クラッシュ）
   - `--in-process-gpu` は使用禁止（白画面クラッシュ）
...
bluetooth-audio          (PipeWire に依存)
  ↓
spi-lcd                  (SPI LCD DRM + Xorg 設定 → kiosk より先に)
  ↓
